...
_TOOL_EXPRESS_BY_PHONE = {
    ...
    "function": {
        ...
        "description": (
            "仅当用户明确询问快递、物流、包裹配送状态（例如'我的快递到哪了'、'查一下物流'）且未指定具体日期时调用。不适用于账号、密码、登录等身份信息类问题。"
...
- 若输入疑似指代日期但表达不够明确（如"那天"、"昨天"、"前天"、"第一个"、"第二个"、纯数字、模糊时间词等），优先调用 express_query_skill_by_date 尝试解析，交由工具内部判断是否有效，而不是直接用文字猜测或拒答
- 只有输入与日期/取消完全无关（如问候、投诉、报修、闲聊等话题）→ 不调用任何工具，只回复"请问您想查哪一天的快递呢？"
- 严禁在未调用工具的情况下，编造任何具体的物流状态、时间、地点等信息
- 只有调用 express_query_skill_by_date 并拿到工具返回结果后，才能提及具体的快递状态
